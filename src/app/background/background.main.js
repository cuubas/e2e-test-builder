export function run() {
  ...
  [['assert', 'Assert'], ['waitFor', 'Wait for'], ['store', 'Store']].forEach(function (accessor) {
    chrome.contextMenus.create({ type: 'separator', contexts: contentContexts, documentUrlPatterns: contentUris });
    // first item is the command suffix, 2nd is the suffix visible to the user
    [['Text', 'Text'], ['Value', 'Value'], ['Visible', 'Visible'], ['ElementPresent', 'Element Present']].forEach(function (cmd) {
